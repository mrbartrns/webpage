...
module.exports = (app, User) => {
  ...
  app.get("/admin/manageusers", auth, admin, (req, res) => {
    User.find()
      .then((users) =>
        res.render("admin_user", { users: users, title: "회원 관리" })
      )
  ...
  app.get("/admin/manageusers/modify/:_id", auth, admin, (req, res) => {
    ...
    User.findOne({ _id: req.params._id })
      .then((user) => {
        res
          .status(200)
          .render("admin_moduser", { user: user, title: "회원 관리" });
  ...
  app.get("/admin/manageboards", auth, admin, (req, res) => {
    Board.find()
      .then((boards) =>
        res.render("admin_boards", { boards: boards, title: "게시판 관리" })
      )
      .catch((err) => res.json({ success: false, err }));
  });
  ...
  app.get("/admin/manageboards/edit/:url", auth, admin, (req, res) => {
    Board.findOne({ url: req.params.url }).then((board) => {
  ...
  app.post("/admin/manageboards/edit/:url", auth, admin, (req, res) => {
    Board.findOneAndUpdate(
      { url: req.params.url },
      { boardName: req.body.boardname, url: req.body.url }
    )
      .then((board) => {
        res.redirect("/admin/manageboards");
      })
